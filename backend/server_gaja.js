...
app.get('/ip', (req, res) => {
        ...
        //call child process (terminal) to exec command whois to get
        //BGP prefix and asname
        let exec = require('child_process').exec;
        let bgp = '';
        let asnName= '';
        let cmd = `whois -h whois.cymru.com " -f -p ${ip}" > output.txt`;
        exec(cmd, (err, stdout, stderr) => {
                ...
                let buffer = fs.readFileSync(path + "/output.txt");
                let text = buffer.toString();
                let extract= text.split('|');
                bgp = extract[2].replace(/ /g, '');
                asnName = extract[3].replace(/ /g, '');
                console.log(bgp);
                console.log(asnName);
        })
